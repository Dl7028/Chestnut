...
class PersistentCookieStore(context: Context) {
    ...
    fun add(url: HttpUrl, cookie: Cookie) {
        ...
        if (cookie.persistent()) {
            ...
            Objects.requireNonNull(cookies[url.host()])!![name] = cookie
        } else {
            if (cookies.containsKey(url.host())) {
                if (cookies[url.host()] != null) {
                    Objects.requireNonNull(cookies[url.host()])?.remove(name)
                }
            }
        }
        if (cookies[url.host()] != null) {
    ...
    operator fun get(url: HttpUrl): List<Cookie> {
        ...
        if (cookies.containsKey(url.host())) {
            ret.addAll(Objects.requireNonNull(cookies[url.host()])!!.values)
        }
        return ret
    }
   ...
   /* fun remove(url: HttpUrl, cookie: Cookie): Boolean {
        val name = getCookieToken(cookie)
        return if (cookies.containsKey(url.host()) && Objects.requireNonNull(cookies[url.host()])
                .containsKey(name)
        ) {
            Objects.requireNonNull(cookies[url.host()]).remove(name)
            val prefsWriter = cookiePrefs.edit()
            ...
            prefsWriter.putString(
                url.host(), Objects.requireNonNull(
                    cookies[url.host()]
                )!!.keys.let {
                    TextUtils.join(
                        ",", it
    ...
    fun getCookies(): List<Cookie> {
        ...
        for (key in cookies.keys) {
            ret.addAll(Objects.requireNonNull(cookies[key])!!.values)
        }
        return ret
    }
     ...
     * @return 序列化之后的string
     */
     fun encodeCookie(cookie: SerializableOkHttpCookies?): String? {
        if (cookie == null) {
            return null
        }
            ...
            for (name in cookieNames) {
                ...
                if (encodedCookie != null) {
                    ...
                    if (decodedCookie != null) {
                        ...
                        Objects.requireNonNull(cookies[key])!![name] =
                            decodedCookie
                    }
                }
            }
